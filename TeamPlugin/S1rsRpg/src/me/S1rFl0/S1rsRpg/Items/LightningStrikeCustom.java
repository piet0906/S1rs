package me.S1rFl0.S1rsRpg.Items;

import java.util.ArrayList;

import org.bukkit.Material;
import org.bukkit.enchantments.Enchantment;
import org.bukkit.inventory.ItemStack;
import org.bukkit.inventory.ShapedRecipe;
import org.bukkit.inventory.meta.ItemMeta;
import org.bukkit.plugin.Plugin;

import me.S1rFl0.S1rsRpg.Main;
import me.S1rFl0.S1rsRpg.Utils.Utils;

public class LightningStrikeCustom {
	public Plugin plugin = Main.getPlugin(Main.class);
	public void customRecipe() 
	{
		ItemStack item = new ItemStack(Material.PRISMARINE_CRYSTALS,1);
		ItemMeta meta = item.getItemMeta();
		
		meta.setDisplayName(Utils.chat("LightningStrike"));
		ArrayList<String> lore = new ArrayList<String>();
		lore.add("Strike The Ground With Lightning");
		meta.setLore(lore);
		meta.setLocalizedName("lightningstrike");
		item.setItemMeta(meta);
		
		@SuppressWarnings("deprecation")
		ShapedRecipe r = new ShapedRecipe(item);
		r.shape("IGI","IPI","IGI");
		r.setIngredient('G', Material.GHAST_TEAR);
		r.setIngredient('I', Material.IRON_INGOT);
		r.setIngredient('P', Material.PRISMARINE_CRYSTALS);
		
		plugin.getServer().addRecipe(r);
	}
	
	public static ItemStack custom() {
		ItemStack item = new ItemStack(Material.PRISMARINE_CRYSTALS,1);
		ItemMeta meta = item.getItemMeta();
		
		meta.setDisplayName(Utils.chat("LightningStrike"));
		ArrayList<String> lore = new ArrayList<String>();
		lore.add("Strike The Ground With Lightning");
		meta.setLore(lore);
		meta.setLocalizedName("lightningstrike");
		item.setItemMeta(meta);
		return item;
	}
	public static ItemStack customE() {
		ItemStack item = new ItemStack(Material.PRISMARINE_CRYSTALS,1);
		ItemMeta meta = item.getItemMeta();

		meta.addEnchant(Enchantment.ARROW_FIRE, 1, true);
		meta.setDisplayName(Utils.chat("Enhanced LightningStrike"));
		ArrayList<String> lore = new ArrayList<String>();
		lore.add("Improved LightningStrike");
		meta.setLore(lore);
		meta.setLocalizedName("lightningstrikee");
		item.setItemMeta(meta);
		return item;
	}
}
